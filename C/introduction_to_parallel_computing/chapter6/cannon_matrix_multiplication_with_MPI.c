...
void MatrixMatrixMultiply(int n, double *a, double *b, double *c, MPI_Comm comm)
{
    ...
    // Get the communicator related inforamtion
    MPI_Comm_size(comm, &npes);
    MPI_Comm_rank(comm, &myrank);

    // Set up the Cartesian topology
    dims[0] = dims[1] = (int) sqrt(npes);

    // Set the periods for wraparound connections
    periods[0] = periods[1] = 1;

    // Create the Cartesian topology, with rank reording
    MPI_Cart_create(comm, 2, dims, periods, 1, &comm_2d);

    // Get the rank and coordinates with respect to the new toppology
    ...
    // Restore the original distrubution of a and b
    MPI_Cart_shift(comm_2d, 0, +mycoords[0], &shiftsource, &shiftdest);
    MPI_Sendrecv_replace(a, nlocal * nlocal, MPI_DOUBLE, shiftdest, 1, shiftsource, 1, comm_2d, &status);
